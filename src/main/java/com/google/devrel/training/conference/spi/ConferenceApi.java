...
public class ConferenceApi {
    ...
    private static Profile getProfileFromUser(User user) {
        ...
        Profile profile = OfyService.ofy().load().key(
...
// To avoid separate datastore gets for each Conference, pre-fetch the Profiles.
    ...
    public List<Conference> getConferencesCreated(final User user) throws UnauthorizedException{
        ...
        Query<Conference> query = OfyService.ofy().load().type(Conference.class).ancestor(profileKey);
        return query.list();
    }
    ...
    public WrappedBoolean unregisterFromConference(
        ...
        if (!result.getResult()) {
            if (result.getReason().contains("No Conference found with key")) {
                ...
                throw new ConflictException("You haven't registered yet");
            }
            else {
                throw new ForbiddenException("Unknown exception");
            }
...
public Collection<Conference> getConferencesToAttend(final User user)
        ...
        for (String conferenceKey :
                keyStringsToAttend) {
            conferences.add(getConference(conferenceKey));
        }
